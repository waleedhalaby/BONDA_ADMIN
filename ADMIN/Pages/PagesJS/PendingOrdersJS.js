        ...
        if(data !== '') {
            ...
            $(orders).each(function (id, order) {
                $('#orderTable').append('<tr>' +
                    ...
                    'onclick="ShowModal(\'Order [#'+order['UNIQUE_ID']+'] Details\',\'Close\',\'Pages/PagesPHP/OrdersPHP/order_details.php?s=1&id='+order['ID']+'\',true)">' +
                    '<i class="halflings-icon white zoom-in"></i></a>'+
                    '</td></tr>'
                );
            });

            if(!CheckPrivilege('UPDATE_PENDING_ORDER')){
                $('#content .box-content').html('Sorry, you don\'t have the privilege to update pending orders.');
            }
        }
        else{
            orders = [];
