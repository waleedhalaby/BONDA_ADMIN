        ...
        if(data !== '') {
            ...
            $(orders).each(function (id, order) {
                $('#orderTable').append('<tr>' +
                    ...
                    'onclick="ShowModal(\'Order [#'+order['UNIQUE_ID']+'] Details\',\'Close\',\'Pages/PagesPHP/OrdersPHP/order_details.php?s=4&id='+order['ID']+'\')">' +
                    '<i class="halflings-icon white zoom-in"></i></a>'+
                    '</td></tr>'
                );
            });

            if(!CheckPrivilege('SHOW_DELIVERED_ORDER')){
