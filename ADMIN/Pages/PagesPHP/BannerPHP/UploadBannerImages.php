        ...
        if ((($_FILES[$id]["type"] == "image/png") || ($_FILES[$id]["type"] == "image/PNG") ||
            ...
            else
            {
                ...
                if(($ID != null || $ID != '') && ($IMAGE != null || $IMAGE != '')){
                    $sql = "SELECT ID,IMAGE_PATH FROM banner_images WHERE ID = '".$ID."' AND IMAGE_PATH ='Assets/Banners/".$_FILES[$id]["name"]."'";
                    ...
                    if($rows > 0){
                        ...
                        $sql = "UPDATE banner_images SET IMAGE_PATH = 'Assets/Banners/".$_FILES[$id]["name"]."' WHERE ID = '".$ID."'";
                        $result2 = mysqli_query($con,$sql);
                        if (!file_exists("../../../Assets/Banners/" . $_FILES[$id]["name"])) {
                            $sourcePath = $_FILES[$id]['tmp_name']; // Storing source path of the file in a variable
                            $targetPath = "../../../Assets/Banners/".$_FILES[$id]['name']; // Target path where file is to be stored
