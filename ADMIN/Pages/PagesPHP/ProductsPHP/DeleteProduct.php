...
if(mysqli_num_rows($result) > 0){
    ...
    $sql = "DELETE FROM designer_products WHERE PRODUCT_ID = ".$PRODUCT_ID;
    $result = mysqli_query($con,$sql);
    ...
    if($result){
        ...
        $sql = "INSERT INTO log_activities (DATE_TIME,PERSON_ID,PAGE_ID,VALUE) VALUES
                                ('".$DATETIME."','".$MAKER_ID."','9','Product [".$PRODUCT_NAME."] is deleted')";
        $result = mysqli_query($con,$sql);
        if($MAKER_ID != 111111){
            $sql = "INSERT INTO notifications (NOTIFY_DATE_TIME,ICON,'COLOR',PAGE_URL,DESCRIPTION,IS_SEEN) VALUES
                                ('".$DATETIME."','icon-gift','pink','Pages/Products.php','Product [".$PRODUCT_NAME."] is deleted','0')";
            $result = mysqli_query($con,$sql);
        }
        echo "Product [".$PRODUCT_NAME."] is deleted successfully.";
    }
}
?>