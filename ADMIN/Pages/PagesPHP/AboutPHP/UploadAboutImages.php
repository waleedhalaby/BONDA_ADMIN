        ...
        if ((($_FILES[$id]["type"] == "image/png") || ($_FILES[$id]["type"] == "image/PNG") ||
            ...
            else
            {
                ...
                if(($ID != null || $ID != '') && ($IMAGE != null || $IMAGE != '')){
                    $sql = "SELECT ID,IMAGE_PATH FROM about_us WHERE ID = '".$ID."' AND IMAGE_PATH ='Assets/About/".$_FILES[$id]["name"]."'";
                    ...
                    if($rows > 0){
                        ...
                        $sql = "UPDATE about_us SET IMAGE_PATH = 'Assets/About/".$_FILES[$id]["name"]."' WHERE ID = '".$ID."'";
                        $result2 = mysqli_query($con,$sql);
                        if (!file_exists("../../../Assets/About/" . $_FILES[$id]["name"])) {
                            $sourcePath = $_FILES[$id]['tmp_name']; // Storing source path of the file in a variable
                            $targetPath = "../../../Assets/About/".$_FILES[$id]['name']; // Target path where file is to be stored
